...
execute if score .main_line ancient_lorras.story matches 17 run tellraw @a[distance=..16] [{"text":"［希爾］我們剛來到古羅拉斯，發現這裡有很多深淵留下的痕跡"}]
execute if score .main_line ancient_lorras.story matches 18 run tellraw @a[distance=..16] [{"text":"［希爾］想問這裡發生了什麼事?"}]
execute if score .main_line ancient_lorras.story matches 19 run tellraw @a[distance=..16] [{"text":"［艾米］我是艾米，在光暗大戰後草之魔劍被深淵侵蝕，在四處留下深淵的痕跡"}]
...
execute if score .main_line ancient_lorras.story matches 21 run tellraw @a[distance=..16] [{"text":"［希爾］那可以告訴我們洞穴的位置嗎?"}]
