...
execute if score .main_line ancient_lorras.story matches 58 run tellraw @a[distance=..16] [{"text":"［小雪］謝謝你們！等我一下，我先把剩下的傷員都包紮傷口一下"}]
execute if score .main_line ancient_lorras.story matches 59 run tellraw @a[distance=..16] [{"text":"(經過一段時間)"}]
execute if score .main_line ancient_lorras.story matches 60 run tellraw @a[distance=..16] [{"text":"［小雪］好了，都結束了"}]
execute if score .main_line ancient_lorras.story matches 61 run tellraw @a[distance=..16] [{"text":"［希爾］剛好時間不多了"}]
execute if score .main_line ancient_lorras.story matches 62 run tellraw @a[distance=..16] [{"text":"［希爾］我們回去櫻花林說明洞穴的事情吧！"}]
