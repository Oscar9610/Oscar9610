...
execute in game_map:ancient_lorras positioned -186.5 261.0 2.5 if score .main_line ancient_lorras.story matches 31 run tellraw @a[distance=..30] [{"text":"［希爾］這裡就是艾米所說的神祕洞穴附近了！"}]
execute in game_map:ancient_lorras positioned -186.5 261.0 2.5 if score .main_line ancient_lorras.story matches 32 run tellraw @a[distance=..30] [{"text":"［希爾］诶？洞穴前面好像有人受傷！我們過去看看！"}]
