...
with open(__file__.replace("item_builder.py","#temp.mcfunction"),mode="w+",encoding="utf-8") as f:
    ...
    f.write(f'{attribute_info(attribute)}],attribute_modifiers={{modifiers:[{attribute_value(attribute)}],show_in_tooltip:false}},custom_model_data={str(item_data["custom_model_data"])},custom_data={str(item_data["custom_data"])},{str(item_data["can_use"])}] 1')
    f.write(backup)