execute if score .spaceship_plain world_tree.story matches 25 run advancement grant @a only task:spaceship/main_mission/1
...
execute if score .spaceship_plain world_tree.story matches 20 run tellraw @a {"text":"【是阿，我能感受到他們身上濃厚的深淵氣息】(點擊繼續對話)","color":"dark_green","clickEvent":{"action":"run_command","value":"/execute if score .spaceship_plain world_tree.story matches 19 run function story:spaceship_plain/1"}}
