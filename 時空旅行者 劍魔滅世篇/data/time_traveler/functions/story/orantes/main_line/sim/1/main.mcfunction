...
execute if score .main_line orantes.story matches 9 run tellraw @s [{"text":"［陳述爺爺］旅行者，請問可以幫我找一下陳述嗎?他最常去的地方是"},{"text":"城市外面的藍色大樹","color":"gold"}]
execute if score .main_line orantes.story matches 10 run tellraw @s {"text":"［陳述爺爺］你們找到陳述我再跟你們說一下魔劍有關的信息"}
execute if score .main_line orantes.story matches 11 run tellraw @s {"text":"【目標】城市外面的藍色大樹找到陳述！","color":"gold"}
