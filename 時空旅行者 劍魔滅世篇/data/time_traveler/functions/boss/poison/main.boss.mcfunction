...
execute if score #hpmax global.main >= #hp global.main unless entity @s[tag=poison_boss.4.already] run function time_traveler:boss/poison/4/use

effect clear @s slowness
execute if block ~ ~-1 ~ slime_block facing entity @p eyes rotated ~ -30 run function time_traveler:entity_fx/motion {range:5}
