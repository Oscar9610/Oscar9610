...
function boss:shadow/tag_loop
execute in game_map:poison_arena run function boss:poison/main
function boss:storm_keeper/tag_loop
function rotas:event/5/shadow/tag_loop
function rotas:boss/infinite_abyss/tag_loop
function ancient_lorras:boss/main/loop

#針對怪物
execute as @e[tag=boss.spawn] at @s run function boss:main/value_set
...
execute as @e[scores={boss.skill.cast.cd=20}] at @s run function boss:main/unlease_skill_tip
...
##根據世界等級計算怪物屬性
# 計算世界等級
execute as @a at @s run function boss:main/world_level
