...
execute if score .main_line ancient_lorras.story matches 35 run tellraw @a[distance=..16] [{"text":"［希爾］你好，我們正在調查關於魔劍與洞穴的事件"}]
execute if score .main_line ancient_lorras.story matches 36 run tellraw @a[distance=..16] [{"text":"［希爾］剛剛看到你正在幫兩位傷員包紮傷口，這個洞穴裡面發生了什麼事？"}]
execute if score .main_line ancient_lorras.story matches 37 run tellraw @a[distance=..16] [{"text":"［？？？］這個洞穴是幻境洞穴，裡面有很多深淵以及路過遭遇襲擊的傷員"}]
execute if score .main_line ancient_lorras.story matches 38 run tellraw @a[distance=..16] [{"text":"［小雪］我的名字叫小雪 我是一名失意的旅者，正在找尋關於過往的自己"}]
execute if score .main_line ancient_lorras.story matches 39 run tellraw @a[distance=..16] [{"text":"［小雪］在到達這個星球時，我恰好來到這裡，偶然看到這裡有一個洞穴"}]
execute if score .main_line ancient_lorras.story matches 40 run tellraw @a[distance=..16] [{"text":"［小雪］上前探索找到了洞穴有很多被怪物襲擊的人，我幫忙擊敗怪物，帶往大家來到入口幫忙包紮傷口"}]
execute if score .main_line ancient_lorras.story matches 41 run tellraw @a[distance=..16] [{"text":"［小雪］入口裡面有很多怪物跟其他村落的傷員在裡面，所以我得趕快進去拯救那些遇難的傷員"}]
execute if score .main_line ancient_lorras.story matches 42 run tellraw @a[distance=..16] [{"text":"［小雪］你們能夠一起幫忙救洞穴裡的傷員嗎?"}]
execute if score .main_line ancient_lorras.story matches 43 run tellraw @a[distance=..16] [{"text":"［希爾］當然可以，我跟旅行者現在就過去洞穴幫忙"}]
