execute as @e[tag=priest.act] at @s if data entity @s interaction.timestamp if score .main_line ancient_lorras.story matches 16..22 run function ancient_lorras:story/main_line/sakura_grove/interaction/guide
