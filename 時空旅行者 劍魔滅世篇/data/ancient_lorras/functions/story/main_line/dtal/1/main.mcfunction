...
execute if score .main_line ancient_lorras.story matches 1 run tellraw @a[distance=..30] {"text":"［艾梅拉］哈囉旅行者，恭喜你完成水星的任務"}
execute if score .main_line ancient_lorras.story matches 2 run tellraw @a[distance=..30] {"text":"［艾梅拉］等等你和希爾還有阿斯卡要去 草星 - 古羅拉斯"}
execute if score .main_line ancient_lorras.story matches 3 run tellraw @a[distance=..30] {"text":"［艾梅拉］古羅拉斯是一個有很多森林跟村落的星球"}
execute if score .main_line ancient_lorras.story matches 4 run tellraw @a[distance=..30] {"text":"［艾梅拉］在那個村落據說有 草之魔劍 - 翠綠茵草 的下落"}
execute if score .main_line ancient_lorras.story matches 5 run tellraw @a[distance=..30] [{"text":"［艾梅拉］但是魔劍的位置目前我們還不知道，可能要你和阿斯卡去村落詢問關於魔劍的情報"}]
